...
def rgb_to_oklab(rgb_array: np.ndarray) -> np.ndarray:
    linear = np.where(
        rgb_array <= 0.04045, rgb_array / 12.92, ((rgb_array + 0.055) / 1.055) ** 2.4
    )
...
def score_base(
    image_oklab: np.ndarray, image_weights: np.ndarray, theme: dict
) -> float:
...
def score_weighted(
    ...
    dist_matrix = (
        cdist(
            image_oklab * weights_array, theme_oklab * weights_array, metric="euclidean"
        )
        * 100
    )
    best_match_distances = np.min(dist_matrix, axis=1)
...
def visualize_3d_clusters(
        ...
        themes_to_plot.append(
            {
                ...
                "label": f"#{i + 1}: {t_name}",
                "hexes": [
                    themes[t_name]["background"],
                    themes[t_name]["foreground"],
                    *themes[t_name]["accents"],
                ],
            }
        )
            ...
            themes_to_plot.append(
                {
                    ...
                    "hexes": [
                        themes[manual_theme]["background"],
                        themes[manual_theme]["foreground"],
                        *themes[manual_theme]["accents"],
                    ],
                }
            )
    ...
    for i, t_info in enumerate(themes_to_plot):
        ...
        accents_rgb = np.array(
            [
                [int(h.lstrip("#")[j : j + 2], 16) / 255.0 for j in (0, 2, 4)]
